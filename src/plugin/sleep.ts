...
// PlayerCore.playerCores that SleepPreventer iterates, so Jellyfin playback lets
...
function spawnCaffeinate(): void {
    const { utils } = iina;
    try {
        utils.exec("caffeinate", ["-d", "-t", String(SLEEP_CAFFEINATE_TIMEOUT_SEC)])
            .then((result) => {
                if (result.status !== 0) {
                    logDebug("Jellyfin: caffeinate exited non-zero:", result.stderr);
                }
            })
            .catch((error) => {
                logDebug("Jellyfin: caffeinate exec failed:", formatError(error));
            });
    } catch (error) {
        logDebug("Jellyfin: caffeinate spawn failed:", formatError(error));
    }
}
